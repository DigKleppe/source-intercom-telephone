...
#define CALL_TIME				300 	// seconds to switch off backlight
#define CHATINTERVAL			1000  	//ms

#define SOFTWAREVERSION 		01   // 2 = 0.2
...
typedef struct {
	uint32_t timeout; // used at other side
	uint32_t keys;
	uint32_t uptime;
	uint32_t errors;
...
typedef enum {STATUS_SHOW_STARTUPSCREEN, STATUS_IDLE, STATUS_RINGING, STATUS_TALKING, STATUS_SETUP, STATUS_TESTMODE,STATUS_SETBELLVOLUME, STATUS_SETSPEECHVOLUME, STATUS_SETHOUSENO, STATUS_ERROR  } status_t;
...
#define CONNECTIONTIMEOUT	60 // seconds
...
#define SETTINGSFILENAME "telefoon.cfg"
