	...
	while (1) {
		...
		case STATUS_SHOW_STARTUPSCREEN:
			...
			case STATUS_RINGING1: // wait until ringcommand is acknowlegded
				...
				if ( commandTimer == 0) {
					//status = STATUS_IDLE; // not received in time, stop
...
//time_t startTime, now, startCall, lastSec; // startCall gaat mis als ie in main staat?????
//
//// and wait until in the air for turning backlight on
//void startMessageThread (void) {
//	if ( !messageThreadStatus.run) {
//		pthread_create(&messageThreadID, NULL, &messageThread, (void *) &messageThreadStatus);
//		while( !messageThreadStatus.run)
//			usleep(10000);
//	}
//
//}
...
//				n = sprintf(message, "\r  Huisnummer: %d\r\n\n", houseNo);
//				n+= sprintf(&message[n], "  Softwareversie: %1.1f\r\n\n", SOFTWAREVERSION * 0.1);
//				sprintf(&message[n], " %s %s", __DATE__ , __TIME__);
//				startMessageThread();
//				subStatus++;
//				backLightOn();
//				break;
...
//
//			case STATUS_IDLE:
//				if ((keysRT & KEY_HANDSET) && !videoThreadStatus.run) {  //handset off  and not showing screen?
//					status = STATUS_SETUP;
//					subStatus = 0;
//				}
//				else {  // horn on the hook
//					if (videoThreadStatus.run) { // still on from last time
//						if (now - startCall >= CALL_TIME) { // video off
//							backLightOff();
...
//					printf("ring base\n");
//					receiveDataBaseFloor.command = COMMAND_NONE;
//					if (!ringThreadStatus.run)  // ring again if not ringing
//						pthread_create(&ringThreadID, NULL, &ringThread, (void *) &ringThreadStatus);
//				}
//				if (receiveDataFirstFloor.command == COMMAND_RING) {
//					floorID = FIRST_FLOOR;
...
//
//				if (keysRT & KEY_HANDSET) { // then horn is off
//					if (ringThreadStatus.run) {  // stop bell
//						ringThreadStatus.mustStop = true;
//						while (ringThreadStatus.run)
//							usleep(1000);
//					}
...
//					videoThreadStatus.mustStop = true;
//					while (audioTransmitThreadStatus.run)
//						usleep(1000);
//					while (audioReceiveThreadStatus.run)
//						usleep(1000);
//					while (videoThreadStatus.run)
//						usleep(1000);
//
